...
An \emph{abstract syntax tree (AST)}~\cite{compiler-construction} is an abstract representation of the structure of program. This representation is stripped of anything that is not essential for the semantics of the program: For exampled, the tree does not contain parenthesis; instead, the order of execution is represented in the order and hierarchy of the tree. Thus, it is not possible to reconstruct the exact source code back from an abstract syntax tree, while it still is possible to rebuild a program that has the same semantics as the original program.
...
The figure also shows that the PHP opcodes also are listed with their opcode name, \eg \emph{T\_CLOSE\_BRACES}.
...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[scale=0.6]{images/parse-tree}
...
\emph{Three-address code (TAC)}~\cite{compiler-construction, compilers} is a theoretical concept for an intermediate program representation that roughly resembles assembly code and that is particularly useful for data-flow analysis. Three-address codes each consist of a basic operation and up to three addresses (hence the name). Addresses (sometimes also called ``places'') can be variables, constants, literals, compiler-generated temporaries, or jump targets.
...
\begin{textcode}
     ...
     (if_neq, t0, 3, @L0) // Jumpo to @L0 if t0 is not equal to 3.
     (=, z, 4)            // z = 4
