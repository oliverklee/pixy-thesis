...
\paragraph{Note:} This only affects assignments by value. Assignments by reference (using the \texttt{=~\&} assigment operator) still are real aliases and are handled accordingly.\index{assign-by-reference}
...
\index{field aliases}\index{member variable aliases \see{field aliases}}
